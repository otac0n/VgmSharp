...
namespace VgmSharp
{
    ...
    /// Leave a <see cref="VgmStreamReader"/> opened with <c>config: null</c> to use vgmstream's own defaults.
    /// </summary>
    public sealed class VgmStreamConfig
    {
        ...
        {
            DisableConfigOverride = this.DisableConfigOverride,
            AllowPlayForever = this.AllowPlayForever,
            PlayForever = this.PlayForever,
            IgnoreLoop = this.IgnoreLoop,
            ForceLoop = this.ForceLoop,
            ReallyForceLoop = this.ReallyForceLoop,
            IgnoreFade = this.IgnoreFade,
            LoopCount = this.LoopCount ?? 0,
            FadeTime = this.FadeTime ?? 0,
            FadeDelay = this.FadeDelay ?? 0,
            StereoTrack = this.StereoTrack,
            AutoDownmixChannels = this.AutoDownmixChannels,
            ForceSfmt = this.ForceSampleFormat.HasValue ? (LibvgmstreamSfmt)this.ForceSampleFormat.Value : 0,
        };
    }
}
